...
@LastEdit  :   2024/04/07 17:31:18
@Author    :   YiboZhao 
@Version   :   1.0
@Site      :   https://github.com/zhaoyib
@RecentInfo:   add the config to adapt the
               lavis models.
'''
import json
import torch
...
class Config():
    ...
    def _load_generic_config(self,configs:dict):
        ...
        load and check generic configs, including following parameters:
            ...
            user_path   : the path to load or save the user data after embedding.
        '''
        ...
        setattr(self,"user_path",configs.get("user_path",""))
        assert self.data_path,\
        "Please input a valid data_path."
        assert self.user_path,\
        "Please input a valid user_path for saving and loading."
    ...
    def _load_train_configs(self,configs:dict):
        ...
        setattr(self,"model_name",configs.get("model_name"))
        assert self.model_name,\
        "Please name the model to distinguish it from others."
        ...
        setattr(self,"display_freq",configs.get("display_freq",2))
        setattr(self,"optimizer_set",configs.get("optimizer_set",{}))
        setattr(self,"trained_model_folder",configs.get("trained_model_folder","autodl-tmp/trained_models/"))
        setattr(self,"ckp_folder",configs.get("ckp_folder","atuodl-tmp/checkpoints/"))
        setattr(self,"ckp_epoch",configs.get("ckp_epoch",5))
        setattr(self,"res_folder",configs.get("res_floder",""))

    def _load_inference_configs(self,configs):
        '''
        load and check configs for inference including following parameters:
            model_name  : path of the model used for inference.
            res_path    : str of the path to store the result.
        '''
        setattr(self,"trained_model_folder",configs.get("trained_model_folder","autodl-tmp/trained_models/"))
        setattr(self,"model_name",configs.get("model_name"))
        assert self.model_name,\
        "Please input the name of model to distinguish which model to load."
        setattr(self,"res_path",configs.get("res_path",""))

        

    def _load_emb_configs(self,configs):
        setattr(self,"using_lavis",configs.get("using_lavis",False))
        if not self.using_lavis:
            ...
            assert isinstance(self.fusion_method,(str,int)),\
            ...
            setattr(self,"lavis_model_name",configs.get("lavis_model_name","blip_feature_extractor"))
            setattr(self,"model_type",configs.get("model_type","base"))
